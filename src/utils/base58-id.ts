...
function assertU64(x: bigint, label: string): bigint {
    ...
    if (x > U64_MAX) throw new Error(`${label}: must be <= 2^64-1`);
    return x;
}
...
function base58Decode(s: string): Uint8Array {
    ...
    for (let i = zeros; i < raw.length; i++) {
        ...
        if (val === undefined) {
            throw new Error(`idParse: invalid base58 character "${ch}"`);
        }
        let carry = val;
...
function idParse(s: string): bigint {
    const raw = s.trim();
    if (!raw) throw new Error("idParse: empty");
    const bytes = base58Decode(raw);
    if (bytes.length > 8) throw new Error("idParse: too large");
